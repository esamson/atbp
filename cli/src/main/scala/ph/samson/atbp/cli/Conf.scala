...
case class Conf(
  ...
  def jiraConf: Option[JiraConf] =
    jira.orElse(confluence.map { case ConfluenceConf(site, user, token) =>
      JiraConf(site, user, token, CustomFields.Default)
    })

  def confluenceConf: Option[ConfluenceConf] = confluence.orElse(jira.map {
    case JiraConf(site, user, token, _) => ConfluenceConf(site, user, token)
  })
}
